import { communityMessageBuilder } from '@api/routes/form/community/communityMessageBuilder';
...
import {
  IVerifyRequestDiscordParams,
  IVerifyRequestParams,
  VerifyRequestFunc,
...
export const communityVerifyHandler: VerifyRequestFunc = async (
  ...
  try {
    const calculatedCheck = getCommunityCheck(
      recordResult.value.id,
      recordResult.value.name,
      recordResult.value.contactDetails,
    );
    if (calculatedCheck != parseInt(params.check)) {
      return {
        isSuccess: false,
        errorMessage:
          'The calculated check value does not match the supplied check value. Maybe the data has been changed?',
        value: anyObject,
      };
    }
  } catch (ex) {
  ...
  const updateResult = await getDatabaseService().community.updateApprovalStatus(
    params.id,
    approvalStatus,
  );
